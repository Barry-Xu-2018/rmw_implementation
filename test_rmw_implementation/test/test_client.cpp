...
TEST_F(TestClient, send_serialized_request_with_bad_arguments) {
  constexpr char service_name[] = "/test";
  const rosidl_service_type_support_t * ts =
    ROSIDL_GET_SRV_TYPE_SUPPORT(test_msgs, srv, BasicTypes);
  ...
  rmw_ret_t ret = rmw_serialize(
    static_cast<void *>(&client_request),
  ...
  ret = rmw_send_serialized_request(nullptr, &serialized_request, &sequence_number);
  EXPECT_EQ(RMW_RET_INVALID_ARGUMENT, ret);
  rmw_reset_error();

  ret = rmw_send_serialized_request(client, nullptr, &sequence_number);
  EXPECT_EQ(RMW_RET_INVALID_ARGUMENT, ret);
  rmw_reset_error();

  ret = rmw_send_serialized_request(client, &serialized_request, nullptr);
  EXPECT_EQ(RMW_RET_INVALID_ARGUMENT, ret);
  rmw_reset_error();
